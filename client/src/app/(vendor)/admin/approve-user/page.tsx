...
const UserApprovalCard = () => {
  ...
  const fetchKycs = async () => {
    const { data } = await axios.get(
      process.env.NEXT_PUBLIC_API_URL + "/kycs?status=pending"
    );
    setKycs(data);
  };
  useEffect(() => {
  ...
  const handleApproval = async (id?: string) => {
    ...
    const { data } = await axios.patch(
      process.env.NEXT_PUBLIC_API_URL + "/kycs/" + id
    );
    if (data) fetchKycs();
  };
      ...
      {kycs.length > 0
        ? kycs.map((item) => {
            return (
              <Card
                key={item._id}
                className="dashboard-card-hover border-1 m-2 shadow-lg bg-gradient-to-br from-white to-gray-50"
              >
                <div className="p-4">
